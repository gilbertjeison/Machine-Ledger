...
namespace FormatoEwo.Outlook
{
    ...
    public class OutlookgGridDefaultGroup : IOutlookGridGroup
    {
        ...
        public virtual string Text
        {
            get {
                if (column == null)
                    return string.Format("Unbound group: {0} ({1})", Value.ToString(), itemCount == 1 ? "1 item" : itemCount.ToString() + " herramienta(s)");
                else
                    return string.Format("{0}: {1} ({2})", column.HeaderText.ToUpper(), Value.ToString(), itemCount == 1 ? "1 item" : itemCount.ToString() + " herramienta(s)"); 
                }
            set { text = value; }
        }
    ...
    public class OutlookGridAlphabeticGroup : OutlookgGridDefaultGroup
    {
        ...
        public override string Text
        {
            get
            {
                return string.Format("Alphabetic: {1} ({2})", column.HeaderText.ToLower(), Value.ToString(), itemCount == 1 ? "1 item" : itemCount.ToString() + " herramienta(s)");
            }
            set { text = value; }
        }
